...
abstract class AbsTree {
	...
	// checking the bst property in tree
	boolean ordered() {// fill in missing code
		
		if(left != null)
		{
			return (value > left.max().value && left.ordered());
			
		}else if(right != null) {
			
			return (value < right.min().value && right.ordered());
...
//			(right == null || value < right.min().value && right.ordered());
	...
	//checking if n is present in tree.
	boolean member(int n) {// fill in missing code
		
		if(value == n) {
			return true;
		}else if (value < n && right != null && right.member(n)) {
			return true;
		}else if(value > n && left != null && left.member(n)) {
			return true;
		}else return false;
...
//			(value > n && left != null && left.member(n));
	...
	public boolean delete(int n) throws Exception {
		...
		Contract.Requires(this.get_count() > 1 || this.right != null || this.left != null);
...
class DupTree extends AbsTree {
	...
	public boolean insert(int n) throws Exception {
		...
		AbsTree absTreeObj = find(n);

		if (absTreeObj != null)
			old_count = absTreeObj.get_count();
		else
			old_count = 0;

		boolean b = super.insert(n);
		// fill in missing code for Contract.Ensures(...)
		//getting the count after insert.
		totalCount = find(n).get_count();

		Contract.Ensures(totalCount == old_count+1);
	...
	Contract.Invariant(ordered());
	// checking if n is present in tree or not
	// getting the count if present in tree.
		if (member(n))
		...
		boolean b = super.delete(n);
			...
			// getting the count after delete
	...
	Contract.Ensures(n_count == old_n_count - 1);
		Contract.Invariant(ordered());
		return b;
	}
...
class AbsTree_Iterator {
	...
	Contract.Requires(root == null || root.ordered());

		stack_tree_nodes(root);

		// fill in missing code for Contract.Ensures(...);
		Contract.Ensures(stack.isEmpty() || stack.peek() == root.min());
	}
	...
	public int next() throws Exception {
		int old_value = value;
		// fill in missing code for Contract.Requires(...);
		Contract.Requires(count >= 0 || this.hasNext());
	...
	private void stack_tree_nodes(AbsTree node) throws Exception {
		...
		// fill in missing code for Contract.Ensures(...)
		Contract.Ensures(node == null || old_node.min() == stack.peek());
	}
